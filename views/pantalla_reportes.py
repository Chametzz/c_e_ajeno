...
class PantallaReportes(Screen):
    ...
    def _build(self):
        ...
        menu = BoxLayout(orientation="vertical", spacing=12, padding=(20, 20))
        ...
        for texto, fn in reportes:
            btn = Button(
                text=texto, size_hint=(1, None), height=52,
                background_color=AZUL, color=BLANCO,
                font_size=15, halign="left",
            )
    ...
    def _rep_est_grupo(self):
        ...
        def ver(x):
            ...
            popup.dismiss()
            self._popup_tabla(
                f"Estudiantes: {grupos[idx].get('materia','')}",
                cols, filas
            )
    ...
    def _rep_mat_carrera(self):
        ...
        def ver(x):
            ...
            filas  = [[m["clave"], m["nombre"], m["creditos"], m["semestre"]]
                      for m in mats]
            popup.dismiss()
    ...
    def _popup_tabla(self, titulo, cols, filas):
        ...
        popup = Popup(title=titulo, content=contenido, size_hint=(0.97, 0.85))
