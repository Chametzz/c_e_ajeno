...
class PantallaReportes(Screen):
    ...
    def _rep_calificaciones(self):
        ...
        filas = [
            [
                ...
                d.get("materia", ""),
                d.get("profesor", ""),
                f"{d['calificacion_parcial']:.1f}"
                if d.get("calificacion_parcial") is not None
                else "—",
                f"{d['calificacion_final']:.1f}"
                if d.get("calificacion_final") is not None
                else "—",
                d.get("periodo", ""),
            ]
            for d in datos
    ...
    def _rep_est_grupo(self):
        ...
        if not grupos:
            popup_mensaje("Info", "No hay grupos registrados.", AZUL)
            return

        contenido = BoxLayout(orientation="vertical", spacing=dp(12), padding=dp(16))
        lbl = Label(
            text="Selecciona grupo:",
            size_hint=(1, None),
            height=dp(32),
            color=NEGRO,
            font_size=dp(15),
        ...
        sp = Spinner(
            values=[
                f"{g['id_grupo']} | {g.get('materia', '')} - {g.get('horario', '')}"
                for g in grupos
            ],
            text="",
            size_hint=(1, None),
        ...
        popup = Popup(
            title="Reporte por grupo", content=contenido, size_hint=(0.92, 0.38)
        )
        ...
        def ver(x):
            ...
            popup.dismiss()
            self._popup_tabla(
                f"Estudiantes: {grupos[idx].get('materia', '')}", cols, filas
            )
    ...
    def _rep_mat_carrera(self):
        ...
        if not carreras:
            popup_mensaje("Info", "No hay carreras.", AZUL)
            return

        contenido = BoxLayout(orientation="vertical", spacing=dp(12), padding=dp(16))
        lbl = Label(
            text="Selecciona carrera:",
            size_hint=(1, None),
            height=dp(32),
            color=NEGRO,
            font_size=dp(15),
        ...
        popup = Popup(
            title="Materias por carrera", content=contenido, size_hint=(0.92, 0.36)
        )
        ...
        def ver(x):
            ...
            filas = [
                [m["clave"], m["nombre"], m["creditos"], m["semestre"]] for m in mats
            ]
            popup.dismiss()
    ...
    def _rep_grupos_prof(self):
        ...
        if not profs:
            popup_mensaje("Info", "No hay profesores.", AZUL)
            return
        ...
        popup = Popup(
            title="Grupos por profesor", content=contenido, size_hint=(0.92, 0.36)
        )
