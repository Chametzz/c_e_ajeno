...
class PantallaCarreras(Screen):
    ...
    def _build(self):
        ...
        for i, car in enumerate(carreras):
            ...
            btns = BoxLayout(spacing=dp(3), size_hint=(None, 1), width=dp(210))
            b_edit = Button(
                text="Editar", font_size=dp(13), background_color=AZUL, color=BLANCO
            )
            b_mat = Button(
                text="Materias", font_size=dp(13), background_color=MORADO, color=BLANCO
            )
            b_del = Button(
