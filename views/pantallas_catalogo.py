"""
views/pantallas_catalogo.py
CRUD de Profesores, Materias, Aulas y Periodos (pantallas más compactas).
"""
...
from models.modelos     import (ProfesorModel, MateriaModel,
                                 AulaModel, PeriodoModel, CarreraModel)
from utils.validaciones import validar_correo, validar_fecha, limpiar_texto
from views.widgets      import (BarraSuperior, TablaScroll, TablaEncabezado,
...
# ──────────────────────────────────────────────────
#  Helper: lista de filas con botones editar/borrar
# ──────────────────────────────────────────────────
def _grid_con_botones(registros, columnas_data, on_editar, on_borrar):
    """
    registros       : lista de dicts
    columnas_data   : lista de lambdas/claves que extraen texto de cada registro
    """
    ...
    if not registros:
        grid.add_widget(Label(text="Sin registros", color=NEGRO,
...
class PantallaProfesores(Screen):
    ...
    """def _borrar(self, reg):
        popup_confirmar("Eliminar", f"¿Eliminar a {reg['nombre']}?",
                        lambda: [ProfesorModel.eliminar(reg["id_profesor"]),
                                 self._build()])"""
    def _borrar(self, reg):
    ...
    def _form(self, reg=None):
        ...
        for w in [r1, r2, r3, r4, r5]: contenido.add_widget(w)
        ...
        popup = Popup(title="Profesor", content=contenido,
                      size_hint=(0.9, 0.75))

        def guardar(x):
            if not validar_correo(ti_mail.text):
                popup_mensaje("Error", "Correo inválido.", ROJO); return
            datos = {"num_empleado": limpiar_texto(ti_emp.text),
            ...
            try:
                if reg: ProfesorModel.actualizar(reg["id_profesor"], datos)
                else:   ProfesorModel.crear(**datos)
...
class PantallaMaterias(Screen):
    ...
    def _form(self, reg=None):
        ...
        if reg:
            ...
            car = next((c for c in carreras if c["id_carrera"] == reg.get("id_carrera")), None)
            if car: sp_c.text = car["nombre"]

        popup = Popup(title="Materia", content=contenido, size_hint=(0.9, 0.82))

        def guardar(x):
            id_car = next((c["id_carrera"] for c in carreras if c["nombre"] == sp_c.text), None)
            if not id_car:
                popup_mensaje("Error", "Selecciona una carrera.", ROJO); return
            datos = {"clave": limpiar_texto(ti_clave.text),
                     "nombre": limpiar_texto(ti_nom.text),
                     "creditos": int(ti_cred.text or 0),
                     "semestre": int(ti_sem.text or 1),
                     "id_carrera": id_car}
            try:
                if reg: MateriaModel.actualizar(reg["id_materia"], datos)
                else:   MateriaModel.crear(**datos)
                popup.dismiss(); self._build()
    ...
    def _form(self, reg=None):
        ...
        def guardar(x):
            datos = {"clave": limpiar_texto(ti_clave.text),
                     ...
                     "tipo": sp_t.text}
            try:
                if reg: AulaModel.actualizar(reg["id_aula"], datos)
                else:   AulaModel.crear(**datos)
                popup.dismiss(); self._build()
                popup_mensaje("Listo", "Guardado.", VERDE)
            except Exception as e:
                popup_mensaje("Error", str(e), ROJO)
...
class PantallaPeriodos(Screen):
    ...
    def _form(self, reg=None):
        ...
        popup = Popup(title="Periodo semestral", content=contenido,
                      size_hint=(0.9, 0.72))

        def guardar(x):
            if not (validar_fecha(ti_ini.text) and validar_fecha(ti_fin.text)):
                popup_mensaje("Error", "Fechas inválidas. Usa YYYY-MM-DD.", ROJO); return
            datos = {"nombre": limpiar_texto(ti_nom.text),
                     "fecha_inicio": ti_ini.text,
                     "fecha_fin":    ti_fin.text,
                     "estado":       sp_e.text}
            try:
                if reg: PeriodoModel.actualizar(reg["id_periodo"], datos)
                else:   PeriodoModel.crear(**datos)
                popup.dismiss(); self._build()
                popup_mensaje("Listo", "Guardado.", VERDE)
            except Exception as e:
                popup_mensaje("Error", str(e), ROJO)
